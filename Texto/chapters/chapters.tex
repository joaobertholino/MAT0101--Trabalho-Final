...
Definindo os polinômios simétricos elementares $\sigma_1 = x_1 + x_2$ e $\sigma_2 = x_1x_2$, observamos por comparação com a forma original que:
\begin{align*}
...
Ao expandir o produto das raízes, obtemos a relação geral em termos dos polinômios simétricos elementares $\sigma_k$:
\begin{equation}
...
\begin{equation}
	S_{k} = x_{1}^{k} + x_{2}^{k} + x_{3}^{k} + \dots + x_{n}^{k}
\end{equation}

Onde $x_{1}, x_{2}, \dots, x_{n}$ são raízes de $S(x)=0$. Essa soma pode ser reescrita com polinômios simétricos elementares. Tal relação é um teorema de Identidade de Newton.
...
Como consequência disso, podemos descobrir as somas das potências com base nos resultados das potências anteriores.

\begin{equation}
	S_k = \sigma_{1}S_{k-1} - \sigma_{2}S_{k-2} + \sigma_{3}S_{k-3} - \dots - (-1)^{n}\sigma_{n}S_{k-n}
\end{equation}


\chapter{Aplicações}

Existem aplicações dos polinômios simétricos tanto na física quanto na criptografia, neste texto exploraremos sua utilidade nas olimpíadas de conhecimento. A seguir temos um exemplo de questão que podemos utilizar as propriedades desenvolvidas até aqui.
...
Se observarmos o sistema podemos utilizá-lo para escrever as relações de Girard para um polinômio do terceiro grau, tal que:

\begin{gather*}
    x^3 - \sigma_1x^2 + \sigma_2x - \sigma_3 = 0 \\
    \text{Substituindo $\sigma$ pelos respectivos valores, temos: }\\
    x^3 - 6x^2 + 11x - 6 = 0 \\
    \text{Podemos reescrevê-lo da seguinte forma: }\\
    x^3 - 5x^2 + 6x -x^2 + 5x - 6 = 0 \\
    x(x^2 - 5x + 6) -x^2 + 5x - 6 = 0 \\
    (x - 1) (x^2 - 5x + 6) = 0\\
    \text{O que por fim nos dá o seguinte resultado: } \\
    (x - 1) (x - 2) (x - 3) = 0
\end{gather*}

Portanto, as soluções do sistema são:
\begin{align*}
	S_{(x, y, z)} &= \{(1, 2, 3); (1, 3, 2); (2, 1, 3); (3, 1, 2); (2, 3, 1); (3, 2, 1)\}
\end{align*}

Dessa forma, transformamos um sistema de 3 variáveis em uma equação de 3° grau e investigamos suas raízes. Nota-se, também, que se L é a quantidade de soluções de um polinômio simétrico, então L = n!, basta notar que há n possibilidades para a primeira posição, n-1 para a segunda, e assim sucessivamente.

Como visto no decorrer do texto as Relações de Girad e as Somas de Potências de Newton tem uma forte ligação tendo em vista que são dependentes das propriedades de simetria.
